    ...
    try:
        async with db.connect() as conn:
            ...
            cursor = await conn.execute(
                f"SELECT {', '.join(FIELDS)} FROM profiles WHERE user_id = ?", (user_id,)
            )
...
async def save_profile(user_id: str, **updates: list[str] | None) -> dict[str, list[str]]:
    ...
    merged = dict(current)
    for field in FIELDS:
        additions = _clean(updates.get(field))
        if not additions:
            continue
        # Append rather than replace: a turn that mentions one new pan should
        # not wipe the rest of the kitchen.
        combined = current.get(field, []) + [a for a in additions if a not in current.get(field, [])]
        merged[field] = combined[-MAX_ENTRIES_PER_FIELD:]
    ...
    try:
        async with db.connect() as conn:
            await conn.execute(
                ...
                """,
                (user_id, *(db.encode(merged[field]) for field in FIELDS)),
            )
            await conn.commit()
    except Exception:
