...
async def test_a_new_chat_forgets_the_thread_but_not_the_user(
    ...
    The failure worth guarding is the tempting one — reusing the delete-everything
    route, or clearing the list only in the browser. The first forgets their
    kitchen, the second forgets nothing at all and lets the model answer the next
    message with the old conversation still in front of it.
    """
    model = patch_model(
        scripted_model(ai("Bagna cauda."), ai("Still bagna cauda."), ai("Fresh start."))
    )
