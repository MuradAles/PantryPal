...
2. LLM-driven tool use on LangGraph. The model decides when to call tools. The tools are `web_search` (external), `read_profile`, and `update_profile`. No hardcoded call sequences.
...
7. Two-tier model routing with streaming. A small fast model for simple turns, a stronger one for tool-using or multi-constraint turns.
...
**Saved favourites.** Raised in every interview, which is real signal, but it is CRUD. It demonstrates none of what this build is being evaluated on, and the profile store makes it a small addition later.
