...
The classifier sits on the same id as the fast tier, which weakens the per-model quota argument for splitting them. `.env.example` still carries a comment claiming the classifier deliberately sits on a different model from the main tiers. That comment is wrong, and the fix is to point `MODEL_CLASSIFIER` somewhere else rather than to edit the comment, since the reasoning behind it was sound.
