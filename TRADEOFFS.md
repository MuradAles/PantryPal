...
**The medical write guard is a denylist, and a denylist has a coverage limit.** This is the part of the build I point at as structural rather than instructed, so it deserves the caveat stated plainly. `MEDICAL_TERMS` is a finite list. However many patterns are in it, there will always be a way to phrase a condition that is not, and if the model calls `remember_about_user` with one of those the phrase reaches SQLite verbatim.

An earlier version of the list was about forty patterns and missed most of the common ones. "lactose intolerant", "acid reflux", "on warfarin", "no gallbladder", "anaphylaxis", "autoimmune" and "asthma" all persisted verbatim, which a review caught. The list is now roughly seventy patterns and covers those, largely by matching on the shape of a statement rather than on each condition: "intolerant" and "intolerance" as standalone signals cover lactose, gluten, histamine and every future one without enumerating them. That closes the specific misses. It does not change the shape of the problem.

Two things narrow the remaining exposure without closing it. Most of these phrasings classify as MEDICAL at the policy layer and never reach the agent, so the model is rarely in a position to try. And the schema has no medical column, so what lands is a stray string in `dislikes` rather than a health record in a field designed to hold one. Neither is the guarantee the list is supposed to provide.

Widening it is genuinely delicate in a way that is worth spelling out, because this list also strips profile writes. "gluten" and "lactose" have to stay storable as bare ingredient names, which is the whole memory design, where an allergy persists as an ingredient to avoid rather than as a diagnosis. A denylist that swallows the ingredient along with the condition breaks the feature it exists to protect. The same trap catches ordinary food words: a bare "heart" would eat "artichoke hearts", and a bare "bypass" turns out to eat "bypass the oven", which is why the list carries "heart condition" and "gastric bypass" instead. There is a canary test of harmless words guarding exactly this, and it caught the "bypass" mistake before it shipped.

The real fix is to classify the phrase before storing it rather than pattern-match it, which is a model call on the write path and was out of window.
...
**Fixed: the medical denylist missed most of the common conditions.** The list caught "diabetes" and "pregnant" and let "lactose intolerant", "acid reflux", "on warfarin", "anaphylaxis", "autoimmune" and "asthma" through to storage verbatim. The mechanism was never the problem, since it sits on the only write path and mutation-checking it turns 36 tests red. The list was. It is now about seventy patterns, and there is a regression test carrying every phrase the review found. See the fuller entry under Known issues, which is where the honest version of this lives: the misses are closed, the shape of the problem is not.
