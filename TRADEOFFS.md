...
**The medical write guard is a denylist, and a denylist has a coverage limit.** This is the part of the build I point at as structural rather than instructed, so the caveat belongs here stated plainly. `MEDICAL_TERMS` is a finite list, and however many patterns are in it there is always another way to phrase a condition. If the model calls `remember_about_user` with one of those, the phrase reaches SQLite verbatim.

An earlier version ran to about forty patterns and missed most of the common ones. "lactose intolerant", "acid reflux", "on warfarin", "no gallbladder", "anaphylaxis" and "asthma" all persisted, which a review caught. It is now roughly seventy, and it closes those mostly by matching the shape of a statement rather than each condition: "intolerant" and "intolerance" as standalone signals cover lactose, gluten, histamine and whatever comes next without enumerating any of them. Two things narrow what is left. Most of these phrasings classify as MEDICAL at the policy layer and never reach the agent, and the schema has no medical column, so a survivor lands as a stray string in `dislikes` rather than as a health record in a field built to hold one. Neither is the guarantee the list is supposed to provide.

Widening it is more delicate than it sounds, because the same list strips profile writes. "gluten" and "lactose" have to stay storable as bare ingredient names, since that is the whole memory design: an allergy persists as an ingredient to avoid, not as a diagnosis. A denylist that swallows the ingredient along with the condition breaks the feature it exists to protect. Ordinary food words sit in the same trap. A bare "heart" eats "artichoke hearts", and a bare "bypass" eats "bypass the oven", which is why the list carries "heart condition" and "gastric bypass" instead. A canary test of harmless words guards this, and it caught the "bypass" mistake before it shipped. The real fix is to classify the phrase on the write path rather than pattern-match it, which is a model call and was out of window.
...
A read-only reviewer audited the committed code near the end of the build, and found more than I expected it to. Everything it raised is below with what was done about it. Where an entry overlaps something described earlier, this is the short version and the fuller account is up there.

The pattern worth noticing across these: not one of them was caught by the test suite, and several were sitting under tests that ran green through the exact code path they broke.
...
**Fixed: the medical denylist missed most of the common conditions.** The mechanism was never the problem, since it sits on the only write path and mutation-checking it turns 36 tests red. The list was, and it is now about seventy patterns with a regression test carrying every phrase the review found. The Known issues entry above has the detail, including why a longer list is still a finite one.

**Fixed: the capped tool loop sent the model a tool call nothing had answered.** When a turn hits the five-iteration ceiling the tools node is skipped by design, so the last message is a request that never got a result, and it was passed straight to a model with no tools bound. That is malformed by the provider's own contract, on the one code path whose entire job is to guarantee the user gets an answer rather than silence. The unanswered tail is now dropped before the final call. Worth being precise about what is and is not verified here: the message list is verified, the provider rejecting it is inference, because confirming it would cost a live API call and the key is capped at twenty a day.

**Fixed: a reply with no text produced an empty bubble and no error.** Found alongside the above. Nothing had raised, so there was nothing to report as an error, but the model had produced no prose either and the user got a blank message. There is now a floor: no text and no failure means one plain sentence instead of silence.

**Fixed: a cold-start race in the graph builder.** Concurrent first requests each opened a checkpointer connection and all but one were leaked. Five callers, five connections, four orphaned. Bounded in practice, since it only fires while the cache is cold, but the leaked connections were invisible to the shutdown path that exists specifically to stop a non-daemon connection holding the process open past `compose stop`. Now guarded by a lock with a re-check inside it.
